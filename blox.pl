...
get_cell((S, _), Pos, T) :- member((Pos, [T, _, _]), S); member((Pos, T), S).
...
set_switch((S, O), Pos, Switch, Func, Positions, ([(Pos, [Switch, Func, Positions]) | S], O)). 
...
dfs(S, _, []) :- is_final(S).
dfs(S, Visited, [Move | Moves]) :-
    move_direction(Move),
    move(S, Move, SNext),
    \+ member(SNext, Visited),
    dfs(SNext, [SNext | Visited], Moves).
